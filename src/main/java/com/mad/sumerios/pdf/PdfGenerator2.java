...
// Filas de datos
            ...
            // Agregar detalles con alineación correcta en una sola celda
            detallesTable.addCell(createCell("Detalle", boldFont, Element.ALIGN_JUSTIFIED, PdfPCell.NO_BORDER));
            detallesTable.addCell(createAlignedCell("Total A:", "$" + formatCurrency(uf.getEstadoCuentaUfDTO().getTotalA()), regularFont));
    ...
    // Sobrecarga para celdas sin fondo
    private static PdfPCell createCell(String content, Font font, int alignment, int border) {
        return createCell(content, font, alignment, border, null);
    }
